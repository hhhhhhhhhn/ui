...
func main() {
	...
	root := 
		NewGridLayout([]GridLayoutArg{
			...
			{NewButton().SetFont(f).SetFontSize(50).SetOutlineColor(Color{1,150,1,255}).SetOutlineThickness(2).SetBackgroundColor(Color{1,1,1,255}).SetHoverBackgroundColor(Color{50,50,50,255}).SetColor(Color{255,255,255,255}).SetContent("Button").OnClick(func(){scroll.SetScroll(scroll.GetScroll() + 1)}), 0, 0, 1, 1},
			...
			{NewSlider().SetBackgroundColor(Color{1,1,1,255}).SetWidth(5).SetSliderColor(Color{100,100,1,255}).Change(0.5).OnChange(func(value float32) {scroll.SetScroll(value * 1000 - 500)}), 2, 2, 2, 2},
